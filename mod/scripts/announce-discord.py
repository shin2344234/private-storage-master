...
page and the GitHub release. Write that file first; the script refuses without it, and refuses if either link is missing.
...
def version():
    ...
    return re.search(r'PSM_VERSION\s+"([^"]+)"', header).group(1)
...
def post(token, channel, content):
    ...
    req = urllib.request.Request(url, data=json.dumps({"content": content}).encode("utf-8"),
                                 headers={"Content-Type": "application/json",
                                          ...
                                          "User-Agent": "PrivateStorageMaster-announce"})
...
def main():
    ...
    args = ap.parse_args()
    if not PAGE_ID:
        print("PAGE_ID is empty. Create the Nexus page first, then put its number in announce-discord.py.")
        return 3
    ver = version()
